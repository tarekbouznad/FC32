...
float* BSP_GYRO_CAL(float* sum)
{
	...
	for (int cal_int = 0; cal_int < 2000 ; cal_int ++) {                                  //Take 2000 readings for calibration.
				BSP_GYRO_GetXYZ(pfDataGyro);                                                             //Read the gyro output.
		        sum[0]+=pfDataGyro[0];
		        sum[1]+=pfDataGyro[1];
		        sum[2]+=pfDataGyro[2];
		        ...
		        sum[0] /= 2000;                                                           //Divide the roll total by 2000.
		        sum[1] /= 2000;                                                           //Divide the pitch total by 2000.
		        sum[2] /= 2000;                                                           //Divide the yaw total by 2000.
		        return sum;
}
